...
async function bootstrap(openid, payload) {
  const item = await familyFor(openid); if (!item || item.deletedAt) throw new Error("ACCESS_DENIED");
...
async function joinFamily(openid, payload) { if (await familyFor(openid)) throw new Error("ALREADY_IN_FAMILY"); const code = String(payload.code || "").trim().toUpperCase(); const rows = await db.collection("family_invites").where({ codeHash: digest(code), usedAt: null }).limit(1).get(); const candidate = rows.data[0]; if (!candidate) throw new Error("INVITE_INVALID"); await db.runTransaction(async transaction => { const inviteResult = await transaction.collection("family_invites").doc(candidate._id).get(); const invite = inviteResult.data; if (!invite || invite.usedAt || new Date(invite.expiresAt).getTime() < Date.now()) throw new Error("INVITE_INVALID"); const familyResult = await transaction.collection("families").doc(invite.familyId).get(); const item = familyResult.data; if (!item || (item.adminOpenids || []).length >= 2) throw new Error("PARENT_LIMIT_REACHED"); await transaction.collection("family_invites").doc(invite._id).update({ data: { usedAt: now(), usedByOpenid: openid } }); await transaction.collection("families").doc(item._id).update({ data: { memberOpenids: _.push(openid), adminOpenids: _.push(openid), updatedAt: now() } }); }); await db.collection("profiles").doc(`${candidate.familyId}-parent-${crypto.randomBytes(4).toString("hex")}`).set({ data: { familyId: candidate.familyId, ownerOpenid: openid, role: "parent", createdAt: now(), updatedAt: now() } }); return { joined: true }; }

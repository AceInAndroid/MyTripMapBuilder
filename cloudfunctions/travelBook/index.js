...
async function listDestinations(openid, payload) { const item = await family(openid); const rows = await db.collection("destinations").where({ familyId: item._id, archivedAt: null }).orderBy("updatedAt", "desc").limit(100).get(); return rows.data.filter(value => !payload.status || value.status === payload.status).map(value => Object.assign({}, value, { id: value._id })); }
async function getDestination(openid, payload) { const item = await family(openid); const destination = await doc("destinations", payload.destinationId); if (!destination || destination.familyId !== item._id || destination.archivedAt) throw new Error("DESTINATION_NOT_FOUND"); const rows = await db.collection("destination_items").where({ familyId: item._id, destinationId: destination._id, deletedAt: null }).orderBy("updatedAt", "desc").limit(500).get(); return { destination: Object.assign({}, destination, { id: destination._id }), items: rows.data.map(value => Object.assign({}, value, { id: value._id })) }; }
async function saveDestination(openid, payload) { const item = await family(openid); admin(item, openid); const source = clean(payload.destination || {}); const destinationId = source.id || id("destination"); const existing = await doc("destinations", destinationId); if (existing && existing.familyId !== item._id) throw new Error("FORBIDDEN"); const timestamp = now(); await db.collection("destinations").doc(destinationId).set({ data: Object.assign({}, source, { familyId: item._id, _id: destinationId, status: source.status || "inbox", archivedAt: null, createdAt: existing ? existing.createdAt : timestamp, updatedAt: timestamp }) }); return { id: destinationId }; }
async function archiveDestination(openid, payload) { const item = await family(openid); admin(item, openid); const value = await doc("destinations", payload.destinationId); if (!value || value.familyId !== item._id) throw new Error("FORBIDDEN"); await db.collection("destinations").doc(value._id).update({ data: { archivedAt: now(), updatedAt: now() } }); return { archived: true }; }
async function saveDestinationItem(openid, payload) { const item = await family(openid); admin(item, openid); const source = clean(payload.item || {}); const destination = await doc("destinations", source.destinationId); if (!destination || destination.familyId !== item._id) throw new Error("DESTINATION_NOT_FOUND"); const itemId = source.id || id("destination-item"); const existing = await doc("destination_items", itemId); if (existing && existing.familyId !== item._id) throw new Error("FORBIDDEN"); const timestamp = now(); await db.collection("destination_items").doc(itemId).set({ data: Object.assign({}, source, { familyId: item._id, _id: itemId, status: source.status || "inbox", type: source.type || "note", verifyRequired: source.verifyRequired !== false, deletedAt: null, createdAt: existing ? existing.createdAt : timestamp, updatedAt: timestamp }) }); return { id: itemId }; }
async function moveDestinationItem(openid, payload) { const item = await family(openid); admin(item, openid); const value = await doc("destination_items", payload.itemId); if (!value || value.familyId !== item._id) throw new Error("FORBIDDEN"); if (["inbox", "to_plan", "planned", "visited"].indexOf(payload.status) < 0) throw new Error("INVALID_DESTINATION_STATUS"); await db.collection("destination_items").doc(value._id).update({ data: { status: payload.status, updatedAt: now() } }); return { moved: true }; }
async function deleteDestinationItem(openid, payload) { const item = await family(openid); admin(item, openid); const value = await doc("destination_items", payload.itemId); if (!value || value.familyId !== item._id) throw new Error("FORBIDDEN"); await db.collection("destination_items").doc(value._id).update({ data: { deletedAt: now(), updatedAt: now() } }); return { deleted: true }; }
async function linkTripToDestination(openid, payload) { const item = await family(openid); admin(item, openid); const destination = await doc("destinations", payload.destinationId); const t = await doc("trips", payload.tripId); if (!destination || !t || destination.familyId !== item._id || t.familyId !== item._id) throw new Error("FORBIDDEN"); const key = `${destination._id}-${t._id}`; await db.collection("destination_trip_links").doc(key).set({ data: { familyId: item._id, destinationId: destination._id, tripId: t._id, relationType: payload.relationType || (t.status === "completed" ? "completed" : "planned"), sortOrder: payload.sortOrder || 0, createdAt: now() } }); return { linked: true }; }
async function unlinkTripFromDestination(openid, payload) { const item = await family(openid); admin(item, openid); const key = `${payload.destinationId}-${payload.tripId}`; const value = await doc("destination_trip_links", key); if (!value || value.familyId !== item._id) throw new Error("FORBIDDEN"); await db.collection("destination_trip_links").doc(key).remove(); return { unlinked: true }; }

exports.main = async event => { const { OPENID } = cloud.getWXContext(); const action = event && event.action; const payload = event && event.payload || {}; try { if (!OPENID && action !== "getShare") return fail("WECHAT_IDENTITY_REQUIRED", "WECHAT_IDENTITY_REQUIRED"); if (action === "bootstrap") return ok(await bootstrap(OPENID, payload)); if (action === "saveTrip") return ok(await saveTrip(OPENID, payload)); if (action === "saveProfile") return ok(await saveProfile(OPENID, payload)); if (action === "createInvite") return ok(await createInvite(OPENID)); if (action === "createReviewInvite") return ok(await createReviewInvite(OPENID)); if (action === "revokeReviewAccess") return ok(await revokeReviewAccess(OPENID)); if (action === "joinFamily") return ok(await joinFamily(OPENID, payload)); if (action === "deleteTrip") return ok(await deleteTrip(OPENID, payload)); if (action === "listDeleted") return ok(await listDeleted(OPENID)); if (action === "restoreTrip") return ok(await restoreTrip(OPENID, payload)); if (action === "createShare") return ok(await createShare(OPENID, payload)); if (action === "getShare") return ok(await getShare(payload)); if (action === "listShares") return ok(await listShares(OPENID)); if (action === "revokeShare") return ok(await revokeShare(OPENID, payload)); if (action === "listDestinations") return ok(await listDestinations(OPENID, payload)); if (action === "getDestination") return ok(await getDestination(OPENID, payload)); if (action === "saveDestination") return ok(await saveDestination(OPENID, payload)); if (action === "archiveDestination") return ok(await archiveDestination(OPENID, payload)); if (action === "saveDestinationItem") return ok(await saveDestinationItem(OPENID, payload)); if (action === "moveDestinationItem") return ok(await moveDestinationItem(OPENID, payload)); if (action === "deleteDestinationItem") return ok(await deleteDestinationItem(OPENID, payload)); if (action === "linkTripToDestination") return ok(await linkTripToDestination(OPENID, payload)); if (action === "unlinkTripFromDestination") return ok(await unlinkTripFromDestination(OPENID, payload)); return fail("Unsupported action", "UNSUPPORTED_ACTION"); } catch (error) { console.error("[travelBook] request failed", { action, message: error && error.message, errCode: error && error.errCode }); return fail(error.message || "Request failed", error.message || "REQUEST_FAILED"); } };

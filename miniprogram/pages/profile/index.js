...
Page({
  data: { profile: null, features: { aiPlannerEnabled: false, aiChatEnabled: false }, editing: false, joining: false, nickname: "", birthday: "", inviteCode: "" },
  onShow: function () { if (this.getTabBar()) this.getTabBar().setData({ selected: 2 }); var self = this; repo.bootstrap().then(function (data) { var profile = data.profile; self.setData({ profile: profile, features: data.features || repo.currentFeatures(), nickname: profile.childNickname, birthday: profile.childBirthday }); }); },

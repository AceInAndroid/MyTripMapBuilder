...
Page({
  ...
  onShow: function () { if (this.getTabBar()) this.getTabBar().setData({ selected: 2 }); var self = this; repo.getProfile().then(function (profile) { if (!profile) return self.setData({ profile: null, joining: true }); self.setData({ profile: profile, joining: false, nickname: profile.childNickname, birthday: profile.childBirthday }); }); },
